...
class TopicIdeatorTab(QWidget):
    def __init__(self):
        ...
        dz_style = DropZoneWidget("📋", "STYLE GUIDE", "Upload để suggest đúng tone")
        dz_dna = DropZoneWidget("🧬", "DNA KÊNH", "Upload để suggest đúng format")
        dz_done = DropZoneWidget("📝", "TOPICS ĐÃ LÀM", "Upload để tránh trùng lặp")
        
        dz_style.file_loaded.connect(lambda p, c: setattr(self, 'style_content', c))
        dz_dna.file_loaded.connect(lambda p, c: setattr(self, 'dna_content', c))
        dz_done.file_loaded.connect(lambda p, c: setattr(self, 'done_content', c))

        ctx_lay.addWidget(dz_style); ctx_lay.addWidget(dz_dna); ctx_lay.addWidget(dz_done)
